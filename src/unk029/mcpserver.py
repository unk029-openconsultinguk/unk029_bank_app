...
SYSTEM_INSTRUCTION = """You are UNK029 Bank's AI assistant. Be friendly, use customer names when known, keep responses concise.
...
4. get_banking_info_tool(query_type): Get general banking information (interest_rates, fees, limits, services, hours, security, opening_account, contact, international, mortgage)

CRITICAL RULES:
1. REMEMBER CONTEXT: If user mentioned an account number earlier in conversation, use it for follow-up requests like "deposit £200" or "withdraw £50"
2. Use customer's name from previous tool results (e.g., if get_account_tool returned name "John", greet them as John)
...
TOOL_DEFINITIONS = {
    "get_account_tool": {
        "description": "Get account info (balance, name). Use when user asks about balance or account.",
    ...
    "get_banking_info_tool": {
        "description": "Get general banking info (interest_rates, fees, limits, services, hours, security, opening_account, contact, international, mortgage).",
...
BANKING_TOOLS = [types.Tool(function_declarations=[
...
def cleanup_old_sessions():
    ...
    now = datetime.now()
    expired = [sid for sid, last in last_activity.items() 
...
def call_bank_api(method: str, endpoint: str, json_data: dict = None) -> dict:
    ...
    try:
        with httpx.Client(timeout=5.0) as http_client:
            ...
            if response.status_code == 200:
                ...
                error_msg = response.json().get("detail", "Request failed") if response.headers.get("content-type", "").startswith("application/json") else "Request failed"
...
QUERY_ALIASES = {
    "rate": "interest_rates", "rates": "interest_rates", "interest": "interest_rates",
    "fee": "fees", "charges": "fees", "cost": "fees",
    "limit": "limits", "maximum": "limits",
    "service": "services", "product": "services", "products": "services",
    "hour": "hours", "time": "hours", "open": "hours", "opening": "hours",
    "secure": "security", "safety": "security", "protection": "security",
    "phone": "contact", "email": "contact", "call": "contact",
    "open_account": "opening_account", "new_account": "opening_account", 
    "create_account": "opening_account", "requirements": "opening_account",
    "abroad": "international", "foreign": "international", "overseas": "international", "currency": "international",
    "home_loan": "mortgage", "house": "mortgage"
}
...
def get_banking_info_tool(query_type: str) -> dict:
    """Get general banking information (rates, fees, limits, services, hours, security, contact, etc)."""
...
async def chat(request: ChatRequest, req: Request):
    ...
    try:
        ...
        if len(recent_history) > 1:
            context_parts = [f"{'User' if m['role'] == 'user' else 'Assistant'}: {m['content']}" 
